...
async function syncdb() {
  ...
  let AstaConn = AstaConnectSock({
    ...
    browser: ["Windows", "chrome", "MSELA-CHUI-V2"],
  ...
  AstaConn.ev.on("connection.update", async (_0x3fd0b3) => {
    ...
    if (_0x2224a7 === "open") {
      ...
      let _0x1f88ec =
        ...
        Config.WORKTYPE +
        "\n  Database: " +
        (isMongodb ? "MongoDb" : sqldb ? "PostegreSql" : "MSELA-CHUI-V2") +
        "\n";
      try {
  ...
  AstaConn.messageId = (length = 8, prefix = "MSELA-CHUI-V2") => {
...
app.listen(randomPort, () =>
  console.log(
    "MSELA-CHUI-V2 Server listening on http://localhost:" + randomPort + "/ "
  )
);
...
async function MakeSession(
  ...
  const officialRepoPath = "/Mselachui03/";
